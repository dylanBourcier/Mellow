...
function SelectFollowers({ followers = [], onChange, value = [] }) {
    ...
    const getSelectedNames = () => {
        return followers
            .filter(follower => selectedFollowers.includes(follower.id))
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            <select 
                ...
                className="hidden"
            >
                {selectedFollowers.map(id => (
                    <option key={id} value={id} selected></option>
                ))}
            </select>
            ...
            {selectedFollowers.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                    {getSelectedNames().map((name, index) => (
                        <span key={index} className="inline-flex items-center px-2 py-1 bg-lavender-1 text-white text-xs rounded-full">
                            {name}
                        </span>
                    ))}
                </div>
                            ...
                            filteredFollowers.map((follower) => (
                                <div
                                    ...
                                    className={`flex items-center justify-between px-3 py-2 cursor-pointer transition-colors ${
                                        isSelected(follower.id) ? 'bg-lavender-1' : ''
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <input
                                            type="checkbox"
                                            checked={isSelected(follower.id)}
                                        ...
                                        />
                                        <span className={`text-sm ${isSelected(follower.id) ? 'font-medium text-white' : 'text-gray-700'}`}>
                                            {follower.name}
                                        </span>
                                    </div>
                                    {isSelected(follower.id) && (
                                        <svg className="w-4 h-4 text-lavender-5" fill="currentColor" viewBox="0 0 20 20">
